...
def pyart(
    ...
    """
    Pyart Wrapper for Hail Accumulation defined by Wallace et al. 2019 and Kalina et al. 2016.
    ...
    mesh_idx : integer
        integer of sweep containing MESH data in the radar object
    fz_level: int
...
def pyodim(
    ...
    """
    Pyodim Wrapper for Hail Accumulation defined by Wallace et al. 2019 and Kalina et al. 2016.
...
def main(
    ...
    )  # Ice Water Content (kg m-3) derived from Ze follow Heymsfield and Miller 1988
    # remove IWC values where hail_hca is not hail
    iwc_sweep[~hail_hca] = 0
    # remove IWC values where temperature is at or below 0
    iwc_sweep[z_sweep > fz_level] = 0

    # pressure correction from Heymsfield and Wright (2014)
    pc = (1000 / pressure) ** 0.545
    # diameter-fall speed relation from Heymsfield and Wright (2014), units of cm/s
